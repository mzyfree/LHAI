...
Environment:
  ...
  WEEKLY_VALID_END        optional; defaults to infer date minus 5 trading days

Training split defaults:
  train: original config start -> 2025-12-31
  valid: 2026-01-05 -> infer date minus 5 trading days
...
LOCAL_LATEST_PACKAGE="${LOCAL_LATEST_PACKAGE:-${LOCAL_MODEL_PACKAGE_DIR}/latest_csi1000_main_model_package.tar.gz}"
LOCAL_WEEKLY_PRED_DIR="${LOCAL_WEEKLY_PRED_DIR:-${OPS_HOME}/preds/csi1000}"
...
derive_valid_end() {
  local infer_date="$1"
  local offset="${WEEKLY_VALID_END_OFFSET:-5}"
...
start_train() {
  ...
  remote "cd '${GPU_PAPER_HOME}' && chmod +x bin/train_weekly_main_models.sh bin/gen_weekly_prod_configs.py && nohup bash bin/train_weekly_main_models.sh '${run_id}' '${remote_provider}' '${TRAIN_END}' '${VALID_START}' '${valid_end}' '${infer_date}' > '${remote_log}' 2>&1 & echo \$! > '${GPU_PROJECT_ROOT}/logs/weekly_train_${run_id}.pid'"
...
collect_train() {
  ...
  local remote_package="${remote_run_root}/csi1000_main_model_packages_${run_id}.tar.gz"
  ...
  ln -sfn "${LOCAL_MODEL_PACKAGE_DIR}/csi1000_main_model_packages_${run_id}.tar.gz" "${LOCAL_LATEST_PACKAGE}"
  ...
  ls -lh "${LOCAL_MODEL_PACKAGE_DIR}/csi1000_main_model_packages_${run_id}.tar.gz"
